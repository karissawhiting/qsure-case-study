...
This is a common table 1 you would often see in manuscripts. Below I am stratifying the data by gender and adding a p value. The specific tests will be specified in the footnote. You can remove the p-values, and overall column if you do not want them there. 

**TODO:** add `by = gender` argument, pipe result into `bold_labels()`, then `add_overall()`, then and `add_p()`

**HINT:** Documentation: [https://www.danieldsjoberg.com/gtsummary/articles/tbl_summary.html#gtsummary-functions-to-add-information]([https://www.danieldsjoberg.com/gtsummary/articles/tbl_summary.html#gtsummary-functions-to-add-information])

```{r}
tbl_summary(df, include = -id, by = gender) 
...
**TODO:** Create a dichotomized version  of `stabilized_glucose` (split by the median) called `gluc_di` using `mutate()` and `ifelse` (or `case_when()`):


**Optional TODO:** Add some gtsummary customizations onto the tbl_uvregression table (bold labels, bold p-values etc.)
...
## <Creat new dichotomized variable here> 
...
**TODO** Use `tbl_uvregression()` to create univariable cox models. Note, you will have to set the `method` argument to `coxph`, and `y` argument to `Surv(time, status)`. See `?tbl_uvregression` help file for an example to follow. 
